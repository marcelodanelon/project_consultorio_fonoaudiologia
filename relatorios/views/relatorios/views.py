...
from relatorios.forms import RelatorioForm_MovimentacaoInsumos, RelatorioForm_Atendimentos
...
def rel_movimentacao_insumos(request):
    form = RelatorioForm_MovimentacaoInsumos()
    form_action = reverse('relatorios:rel_movimentacao_insumos')

    if request.method == 'POST':
        form = RelatorioForm_MovimentacaoInsumos(request.POST)
            ...
            if valor:
                if campo == 'dataEntrada' or campo == 'dataValidade':
                    try:
                        ...
                        return render(
                            request, 
                            'relatorios/rel_visualizacao.html', 
...
def rel_atendimentos(request):
    form = RelatorioForm_Atendimentos()
    form_action = reverse('relatorios:rel_atendimentos')

    if request.method == 'POST':
        form = RelatorioForm_Atendimentos(request.POST)
        resultados = AtendimentoModel.objects.all()
        if form.is_valid():
            campo_agrupamento = form.cleaned_data['campos_agrupamento']
            campos_selecionados = form.cleaned_data['campos']
        ...
        for campo, valor in campos_filtro.items():
            if valor:
                try:
                    resultados = AtendimentoModel.objects.filter(query)
                    break
                except Exception as e:
                    error_message = str(e)
                    if "Field 'id' expected a number but got" in error_message:
